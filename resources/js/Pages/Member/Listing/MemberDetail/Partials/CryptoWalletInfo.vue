...
<template>
    <div class="flex flex-col items-center p-3 md:py-5 md:px-6 gap-3 self-stretch rounded-lg bg-white shadow-card">
        ...
        <div v-if="displayAccounts" class="flex flex-col divide-y divide-gray-200 items-start self-stretch">
            <div
                v-for="(account, index) in displayAccounts"
                :key="index"
                class="flex flex-col items-start self-stretch py-3 first:pt-0 last:pb-0"
            >
                <div class="font-semibold truncate w-full">{{ account.payment_account_name ? account.payment_account_name : '-' }}</div>
                <div class="truncate w-full">{{ account.account_no ? account.account_no : '-' }}</div>
            </div>
        </div>
    </div>
