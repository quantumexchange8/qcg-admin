    ...
    <GuestLayout :title="$t('public.forgot_password')">
        <div v-if="!submitted" class="w-full flex flex-col items-center justify-center gap-8 pt-8 md:pt-0">
            ...
            <form @submit.prevent="submit" class="flex flex-col items-center gap-6 self-stretch">
                ...
                <div class="flex flex-col items-center gap-4 self-stretch">
                    <Button size="base" variant="primary-flat" class="w-full" :disabled="form.processing" @click.prevent="submit">{{ $t('public.send_reset_password_link') }}</Button>
                    <Button size="base" variant="primary-text" class="w-full" :disabled="form.processing" @click.prevent="goToLoginPage">{{ $t('public.back_to_login') }}</Button>
                </div>
            </form>
        </div>
        ...
        <div v-else class="w-full flex flex-col items-center justify-center gap-8 pt-8 md:pt-0">
            ...
            <div class="flex flex-col items-center justify-center gap-6 self-stretch">
                <Button size="base" variant="primary-flat" class="w-full" :disabled="form.processing" @click.prevent="goToLoginPage">{{ $t('public.back_to_login') }}</Button>
