...
<template>
    <AuthenticatedLayout :title="`${$t('public.transactions')}&nbsp;-&nbsp;${$t('public.sidebar_rewards')}`">
        <div class="flex flex-col justify-center items-center px-3 py-5 self-stretch rounded-lg bg-white shadow-card md:p-6 md:gap-6">
            <div class="flex flex-col pb-3 gap-3 items-center self-stretch md:flex-row md:gap-0 md:justify-between md:pb-0">
                <span class="text-gray-950 font-semibold self-stretch md:self-auto">{{ $t('public.all_rewards_redemption') }}</span>
            </div>
            ...
            >
                ...
                <template v-if="transactions?.length > 0 && filteredValue?.length > 0">
                    ...
                    <Column field="reward_code" :header="$t('public.code')" class="w-1/3 md:w-[15%] px-3 ">
                        <template #body="slotProps">
    ...
    <Dialog v-model:visible="visible" modal :header="$t('public.redemption_details')" class="dialog-xs md:dialog-md" :dismissableMask="true">
        <div class="flex flex-col justify-center items-center gap-3 self-stretch pt-4 md:pt-6">
            <div class="flex flex-col justify-between items-center p-3 gap-3 self-stretch bg-gray-50 md:flex-row">
                ...
                <div class="flex items-center self-stretch">
                    <span class="w-full truncate text-gray-950 text-lg font-semibold">{{ `${formatAmount(data?.transaction_amount || 0)}&nbsp;tp` }}</span>
                </div>
            </div>
