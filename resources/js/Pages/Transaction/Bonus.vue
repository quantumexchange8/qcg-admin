...
const getResults = async (selectedMonths = [], selectedTeams = []) => {
    loading.value = true;
...
watch(selectedMonths, (newMonths) => {
    getResults(newMonths, selectedTeams.value);
    // console.log(newMonths)
});

watch(selectedTeams, (newTeams) => {
    getResults(selectedMonths.value, newTeams);
...
<template>
    <AuthenticatedLayout :title="`${$t('public.transactions')}&nbsp;-&nbsp;${$t('public.sidebar_bonus')}`">
        <div class="flex flex-col justify-center items-center px-3 py-5 self-stretch rounded-lg bg-white shadow-card md:p-6 md:gap-6">
            <div class="flex flex-col pb-3 gap-3 items-center self-stretch md:flex-row md:gap-0 md:justify-between md:pb-0">
                <span class="text-gray-950 font-semibold self-stretch md:self-auto">{{ $t('public.all_credit_bonus') }}</span>
                    ...
                    <Column field="transaction_amount" :header="`${$t('public.amount')}&nbsp;($)`" sortable class="w-1/3 md:w-[15%] px-3">
                        <template #body="slotProps">
                            <div class="flex flex-col items-start max-w-full">
                                ...
                                <div class="text-blue-500 text-xs truncate max-w-full">
        ...
        <div class="flex flex-col justify-center items-center gap-3 self-stretch pt-4 md:pt-6">
            ...
            <div class="flex flex-col items-center p-3 gap-3 self-stretch bg-gray-50">
                ...
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    ...
                    <span class="w-full break-all text-gray-950 text-sm font-medium" @click="copyToClipboard('to_meta_login', data.to_meta_login)">
                        {{ data?.to_meta_login || '-' }}
                        <IconCopy 
            ...
            <div class="flex flex-col items-center p-3 gap-3 self-stretch bg-gray-50">
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    <span class="w-full max-w-[140px] truncate text-gray-500 text-sm">{{ $t('public.deposit_amount') }}</span>
                    <span class="w-full truncate text-gray-950 text-sm font-medium">{{ `$&nbsp;${formatAmount(data?.deposit_amount || 0)}` }}</span>
                </div>
                <div class="w-full flex flex-col items-start gap-1 md:flex-row">
                    <span class="w-full max-w-[140px] truncate text-gray-500 text-sm">{{ $t('public.claim_amount') }}</span>
                    <span class="w-full truncate text-blue-500 text-sm font-medium">{{ `$&nbsp;${formatAmount(data?.transaction_amount || 0)}` }}</span>
                </div>
            </div>
