...
const getResults = async () => {
    ...
    try {
        const response = await axios.get('/pending/getPendingBonusData');
...
const recalculateTotals = () => {
    ...
    const filtered = pendingBonus.value.filter(pendingBonus => {
        const matchesGlobalFilter = globalFilterValue 
            ? [
                pendingBonus.user_name, 
        ...
        const matchesNameFilter = !filters.value.user_name?.value || pendingBonus.user_name.startsWith(filters.value.user_name.value);
        const matchesEmailFilter = !filters.value.user_email?.value || pendingBonus.user_email.startsWith(filters.value.user_email.value);

        // Only return pendingBonus that match both global and specific filters
        return matchesGlobalFilter && matchesNameFilter && matchesEmailFilter;
    });

    // Calculate the total for successful pendingBonus
...
const exportXLSX = () => {
    ...
    const rows = data.map(obj => {
        const fromDisplay = obj.from === 'bonus_wallet' ? trans('public.' + obj.from) : obj.from || '';
...
<template>
    <AuthenticatedLayout :title="$t('public.bonus')">
                ...
                >
                    <template #header>
                        <div class="flex flex-col justify-between items-center pb-5 gap-5 self-stretch md:flex-row md:pb-6">
                            <span class="self-stretch md:self-auto text-gray-950 font-semibold">{{ $t('public.bonus_request', {action: ''}) }}</span>
                    ...
                    <template v-if="pendingBonus?.length > 0 && filteredValue?.length > 0">
                        ...
                        <Column field="created_at" :header="$t('public.date')" sortable style="width: 15%" class="hidden md:table-cell">
                            <template #body="slotProps">
                        ...
                        <Column field="deposit_amount" sortable style="width: 15%; max-width: 0;" class="px-3">
                            <template #header>
