...
<template>
    ...
    <ConfirmDialog group="message_only">
        <template #container="{ message, acceptCallback, rejectCallback }">
            <div class="flex flex-col justify-center items-center px-4 pt-4 pb-6 gap-3 bg-white rounded shadow-dialog w-[90vw] md:w-[248px]">
                <div class="flex flex-col items-center gap-2 self-stretch">
                    <!-- <span class="self-stretch text-gray-950 text-center font-bold md:text-lg">{{ message.header }}</span> -->
                    <span class="self-stretch text-gray-700 text-left text-sm">{{ message.message }}</span>
                    <!-- <span class="self-stretch text-gray-700 text-center text-sm">
                        {{ message.text }}
                        <span class="self-stretch text-gray-950 text-sm font-semibold">{{ message.dynamicText }}</span>
                        {{ message.suffix }}
                ...
                <!-- <div class="flex justify-end items-center gap-4 self-stretch"> -->
                <div class="flex flex-row justify-end items-center gap-3 self-stretch">
                    <Button
                        type="button"
                        variant="gray-outlined"
                        @click="rejectCallback"
