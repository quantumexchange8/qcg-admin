...
class RebateController extends Controller
{
    ...
    public function getCompanyProfileData(Request $request)
    {
        ...
        $company_profile = RebateAllocation::with([
            'user' => function ($query) {
                $query->withCount([
                    'directChildren as direct_agent' => function ($q) {
                        $q->where('role', 'agent');
                    }
                ]);
            }
    ...
    public function getAgents(Request $request)
    {
        ...
        // Check if ID 82 exists and move it to the first position
        $id_82_index = array_search(82, array_column($lv1_agents, 'id'));
        if ($id_82_index !== false) {
            $id_82_agent = $lv1_agents[$id_82_index];
            unset($lv1_agents[$id_82_index]);
            array_unshift($lv1_agents, $id_82_agent); // Add to the start
        }
